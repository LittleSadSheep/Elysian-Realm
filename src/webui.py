...
def load_config():
    if not os.path.exists(CONFIG_PATH):
        ...
        cfg = {
            ...
            "infer": {"max_length": 128, "temperature": 1.0},
            "tune": {"n_trials": 10, "save_best": True},
        }
...
def launch_webui():
    ...
    with gr.Blocks(title="Elysian-Realm WebUI") as demo:
        ...
        with gr.Tab("训练"):
            use_optuna = gr.Checkbox(
                label="使用Optuna自动调参", value=cfg["train"].get("use_optuna", False)
            )
            with gr.Row(
                visible=not cfg["train"].get("use_optuna", False)
            ) as manual_params:
                lr = gr.Number(label="学习率", value=cfg["train"]["learning_rate"])
                batch_size = gr.Number(
                    label="Batch Size", value=cfg["train"]["batch_size"]
                )
                epochs = gr.Number(
                    label="训练轮数", value=cfg["train"]["num_train_epochs"]
                )
                lora_r = gr.Number(label="LoRA r", value=cfg["train"]["lora_r"])
                lora_alpha = gr.Number(
                    label="LoRA alpha", value=cfg["train"]["lora_alpha"]
                )
                lora_dropout = gr.Number(
                    label="LoRA Dropout", value=cfg["train"]["lora_dropout"]
                )
                grad_acc = gr.Number(
                    label="梯度累积步数",
                    value=cfg["train"].get("gradient_accumulation_steps", 4),
                )
                fp16 = gr.Checkbox(
                    label="使用FP16混合精度", value=cfg["train"].get("fp16", True)
                )
                grad_ckpt = gr.Checkbox(
                    label="梯度检查点",
                    value=cfg["train"].get("gradient_checkpointing", True),
                )
                dataloader_workers = gr.Number(
                    label="数据加载线程数",
                    value=cfg["train"].get("dataloader_num_workers", 2),
                )
                seed = gr.Number(label="随机种子", value=cfg["train"].get("seed", 42))
                early_stop = gr.Number(
                    label="EarlyStopping耐心",
                    value=cfg["train"].get("early_stopping_patience", 5),
                )
            with gr.Row(visible=cfg["train"].get("use_optuna", False)) as optuna_params:
                n_trials = gr.Number(
                    label="Optuna实验轮数", value=cfg["tune"]["n_trials"]
                )
            save_best = gr.Checkbox(
                label="自动保存最优模型", value=cfg["train"].get("save_best", True)
            )
            ...
            use_optuna.change(
                toggle_params, [use_optuna], [manual_params, optuna_params]
            )
        ...
        with gr.Tab("推理"):
            ...
            infer_btn.click(
                on_infer_click, [infer_input, max_length, temperature], infer_output
            )
        with gr.Tab("调参"):
            n_trials = gr.Number(label="实验轮数", value=cfg["tune"]["n_trials"])
            tune_save_best = gr.Checkbox(
                label="自动保存最优模型", value=cfg["tune"]["save_best"]
            )
